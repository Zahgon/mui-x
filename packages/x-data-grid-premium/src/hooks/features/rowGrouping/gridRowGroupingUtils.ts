    ...
    if (isRowMatchingFilters && node.type !== 'footer') {
      const shouldApplyItem =
        node.type === 'group' && node.isAutoGenerated
          ? (columnField: string) => { throw new Error("STUB"); }
          : undefined;
    ...
    if (isPassingFiltering === false) {
      if (node.type === 'group') {
        ...
        isPassingFiltering = passFilterLogic(
          allResults.map((result) => { throw new Error("STUB"); }),
          allResults.map((result) => { throw new Error("STUB"); }),
          filterModel,
          filterValueGetter,
          params.apiRef,
...
export const setStrategyAvailability = (
  ...
  if (disableRowGrouping) {
    isAvailable = () => { throw new Error("STUB"); };
  } else {
    isAvailable = () => {
        throw new Error("STUB");
    };
...
export const getGroupingRules = ({
  ...
  sanitizedRowGroupingModel.map((field) => { throw new Error("STUB"); });
